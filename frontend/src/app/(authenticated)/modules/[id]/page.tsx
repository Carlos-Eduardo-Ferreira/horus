...
export default function ModuleFormPage() {
  ...
  // Define o título da página de acordo com a módulo (novo ou edição)
  usePageTitle(isNew ? 'Novo Módulo' : 'Editar Módulo');
  ...
  useEffect(() => {
    if (!isNew && id) {
      ...
      if (!token) return;
      modulesService.get(id, token).then(data => {
        setGroups([
          [
  ...
  const handleChange = (name: string, value: string | number) => {
    ...
    if (hasSubmitted) {
      ...
      setFieldErrors(validateModuleForm(allFields));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const errors = validateModuleForm(allFields);
    ...
    try {
      if (isNew) {
        await modulesService.create(payload, token);
      } else {
        await modulesService.update(id, payload, token);
      }
      setSubmitting(false);
      router.push("/modules");
    } catch (error: unknown) {
